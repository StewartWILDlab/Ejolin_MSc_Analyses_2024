...
This document is used to create the datasets to be used in GLMM analyses in the next script, 9d_AllSpatialScales_Weekly_Cleaned.Rmd

This script also:
- compares snow derived and biologically relevant caribou seasons for use in analyses. 
- compared daily and weekly temporal sampling grain for use in analyses.
- tested the significance of camera targets in each spatiotemporal dataset to test whether or not they should be included in analyses.
...
#IMPORTANT: NEED TO RUN THIS SO LAT/LONG DONT GET STANDARDIZED
#non response variable species should be scaled but that can manually be done in the GLMM formulas
library(MuMIn)
z_locs <- stdize(locs, omit.cols = c("latitude","longitude","latitude_site","longitude_site","spatial_scale","X300_TSPNF","X300_SPTNF","X300_WET","X300_WAT","X300_MF","X300_TSPS", "X300_TSPG","X300_SPPSLM","X300_SPPGLM","X300_BAR","X300_TSPBDF","X300_MFBL","X300_NF","TUNDRA","TAIGA","MOIST"))
```

#######################################################################################################################################################################################################################################################################################################################################

# Delineate Caribou Seasons (GPS)

Rather than defining seasons using snow cover (which would be more suited to a multispecies analysis) I want to divide data by biologically relevant "caribou seasons". The problem here is that these seasons vary by date from year to year and from herd to herd based on GPS collar data. I have 2021-2022 dates for the Ahiak herd from GN reports but I don't have similar recent seasonal date ranges for the Bathurst/Beverly herds (date ranges used in many reports for the Bathurst herd are from a 2011 study). I do have telemetry data for the Bev/Bath caribou but it's clipped to the spatial extent of TDN. 
...
If we used these dates, how do we account for the different herds? Ahiak/NE herds winter here but they're expected to winter on the tundra as opposed to Bev/Bath) and the seasons where we're less likely to have female caribou (i.e. summer/spring)
...
## Add Caribou Seasons (Detection Data)

Based on the daily detections it looks like caribou detection rate could be broken down into the following biologically relevant seasons based on natural breaks. These dates roughly conform to date ranges for the Beverly, Bathurst, and Ahiak herds listed in various GN and GNWT reports. 
...
Create datasets to be used in subsequent analyses. Regional extent refers to all cameras across all of TDN
...
Decided to keep the dates based on daily detection rate due to higher resolution.
...
Local extent includes all cameras within clusters where at least one camera had at least one caribou detection at any point in the year, represents the actual home range during this sampling period.
...
The rationale for using caribou detection data to define the local extent is that we want the spatial extent to be biologically relevant to caribou in TDN specifically and from 2021-22 specifically. 
...
```{r}
#multiseason
write.csv(mod_dat_wk, file="Processed_Data/glmm_2_wk_region_multi.csv", row.names = F)
#summer
write.csv(mod_dat_wk_summer, file="Processed_Data/glmm_2_wk_region_smr.csv", row.names = F)
#fall
write.csv(mod_dat_wk_fall, file="Processed_Data/glmm_2_wk_region_fall.csv", row.names = F)
#winter
write.csv(mod_dat_wk_winter, file="Processed_Data/glmm_2_wk_region_wtr.csv", row.names = F)
#spring
write.csv(mod_dat_wk_spri, file="Processed_Data/glmm_2_wk_region_spr.csv", row.names = F)
```

##Local data sets
```{r}
#multiseason
write.csv(mod_dat_wk_cbou, file="Processed_Data/glmm_2_wk_local_multi.csv", row.names = F)
#summer
write.csv(mod_dat_wk_cbou_summer, file="Processed_Data/glmm_2_wk_local_smr.csv", row.names = F)
#fall
write.csv(mod_dat_wk_cbou_fall, file="Processed_Data/glmm_2_wk_local_fall.csv", row.names = F)
#winter
write.csv(mod_dat_wk_cbou_winter, file="Processed_Data/glmm_2_wk_local_wtr.csv", row.names = F)
#spring
write.csv(mod_dat_wk_cbou_spri, file="Processed_Data/glmm_2_wk_local_spr.csv", row.names = F)
```
